...
{
    [BurstCompile]
    public struct NetCubeTranslationPrecisionVariantGhostComponentSerializer
    {
        static GhostComponentSerializer.State GetState()
        {
            ...
            if (!s_StateInitialized)
            {
                ...
                {
                    GhostFieldsHash = 8546849170448464237,
                    ...
                    VariantHash = 6128860104974352098,
                    CopyToSnapshot =
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                snapshot.Value_x = (int) math.round(component.Value.x * 100000);
                snapshot.Value_y = (int) math.round(component.Value.y * 100000);
                snapshot.Value_z = (int) math.round(component.Value.z * 100000);
            }
        }
        [BurstCompile]
        [MonoPInvokeCallback(typeof(GhostComponentSerializer.CopyToFromSnapshotDelegate))]
        private static void CopyFromSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                var Value_Before = new float3(snapshotBefore.Value_x * 1E-05f, snapshotBefore.Value_y * 1E-05f, snapshotBefore.Value_z * 1E-05f);
                var Value_After = new float3(snapshotAfter.Value_x * 1E-05f, snapshotAfter.Value_y * 1E-05f, snapshotAfter.Value_z * 1E-05f);
